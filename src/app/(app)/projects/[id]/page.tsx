...
export default async function ProjectOverviewPage({
  ...
  // transaction list.
  //
  // Spending-over-time buckets are derived from the per-project range
  // (persisted in `projects.spending_range`). The pure helper
  // `computeSpendingBuckets` encapsulates the four branches (7d / 30d /
  // 90d / all), the Sunday-floor that applies ONLY to the 90d weekly
  // branch, the >180-day stride switch (weekly → monthly) for the
  // all-time branch, and the 0-txn empty case — all pinned by Vitest.
  // The selection only pivots the spending-over-time trend chart; the
  // category donut, paid-vs-consumed bars, Sankey, SettleUpCard and
  // BalanceView all span the full project timeline.
  // -----------------------------------------------------------------------
  const spendingBucketMs = computeSpendingBuckets(
    project.spendingRange,
    projectTransactions,
    new Date(),
  );
